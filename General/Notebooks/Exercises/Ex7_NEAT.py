...
TRACK = pygame.image.load("pygame_assets/track.png")
...
class Car(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.original_image = pygame.image.load("pygame_assets/car.png") # we need it then for rotation
...
def eval_genomes(genomes, config):
    ...
    def from_vector_to_direction(vector):

        dirs_trans = ["LEFT", "STRAIGHT", "RIGHT"]
        dirs = np.array([[-1, 0], [0, 1], [1, 0]])
        cos_sim = [np.dot(np.array(vector), d)/(np.linalg.norm(np.array(vector))*np.linalg.norm(d)) for d in dirs]

        print(f"VECTOR {vector} mapped in {dirs_trans[np.argmin(cos_sim)]}")

        return dirs_trans[np.argmin(cos_sim)]
    ...
    while run:
        ...
        t += 1 # this is just the eta as fitness (not a good one)
        ...
        for i, genome in enumerate(ge):
            ...
            cars[i].update()
            action = nets[i].activate(cars[i].get_input())
            cars[i].direction = from_vector_to_direction(action)
        ...
        cars_display = pygame.sprite.Group(cars)
        cars_display.draw(SCREEN)
